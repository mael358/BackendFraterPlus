...
public interface IFacturaDao extends CrudRepository<Factura, Long> {
    ...
    @Query("select fc from Factura fc where fc.tipo_factura = false")
    List<Factura> findAllByTipoFacturaCompra();

    @EntityGraph(attributePaths = {"detalle_factura", "detalle_factura.articuloid","cliente_id"})
    @Query("select fc from Factura fc where fc.tipo_factura = true")
    List<Factura> findAllByTipoFacturaVenta();
